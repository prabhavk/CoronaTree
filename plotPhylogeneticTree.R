...
coronaTree_all <- root(coronaTree_all,outgroup = coronaTree_all$tip.label[399],resolve.root = T)
...
coronaTree_all <- ladderize(coronaTree_all,right=F)
png(filename = "coronaTree",width=1000,height=800)
...
write.table(treeDistances,file = "tree_distances",sep = "\t",row.names = T,col.names = T)
